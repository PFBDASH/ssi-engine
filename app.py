...
# These let you change slugs without touching code again
WEBFLOW_LOGIN_PATH = os.getenv("WEBFLOW_LOGIN_PATH", "/login").strip()   # e.g. /sign-in
WEBFLOW_SIGNUP_PATH = os.getenv("WEBFLOW_SIGNUP_PATH", "/plans").strip() # e.g. /plans
...
SIGNUP_URL = build_webflow_url(WEBFLOW_SIGNUP_PATH)
...
def verify_memberstack_token(token: str) -> Optional[Dict[str, Any]]:
   ...
   IMPORTANT FIX:
   - Endpoint is /members/verify-token (NOT /members/verifyToken)
   """
...
def get_member(payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
   """
   Memberstack verify-token may return:
   - { member: {...} }
   - { data: { member: {...} } }
   - or other wrappers
   """
